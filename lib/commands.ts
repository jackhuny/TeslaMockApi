...
export const sentryMode = async (
	...
	try {
		await setStateValue(vehicleId, "vehicle_state", "sentry_mode", String(on));
...
export const climateMode = async (
	vehicleId: string,
	...
	try {
		await setStateValue(
			...
			"climate_state",
			"is_climate_on",
			climate_keeper_mode != 0 ? "true" : "false"
		);
		await setStateValue(
			vehicleId,
			"climate_state",
...
export const actuateTrunk = async (
	...
	try {
		if (which_trunk == "front") {
			const currentState = await getState(vehicleId, "vehicle_state", "ft");
			await setStateValue(
				vehicleId,
				"vehicle_state",
				"ft",
			...
			const currentState = await getState(vehicleId, "vehicle_state", "rt");
			await setStateValue(
				vehicleId,
				"vehicle_state",
				"rt",
